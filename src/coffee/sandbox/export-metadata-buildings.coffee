...
load_buildings = (buildings_dir) ->
  new Promise (done, error) ->
    try
      ...
      definitions = _.map(FileUtils.parse_to_json(buildings_dir, ['.json'], ['-image.json', '-simulation.json']), STARPEACE.building.BuildingDefinition.from_json)
      simulation_definitions = _.map(FileUtils.parse_to_json(buildings_dir, ['-simulation.json'], []), STARPEACE.building.simulation.BuildingSimulationDefinitionParser.from_json)
