...
class LandTexture
  constructor: (@directory, @file_path, @image) ->
    @hash = LandTexture.image_hash(@image.bitmap.width, @image.bitmap.height, @image.bitmap.data)
    @map_color = LandTexture.calculate_map_color(@image.bitmap.width, @image.bitmap.height, @image.bitmap.data)
  ...
  key: () ->
    "land.#{@id.toString().padStart(3, '0')}.#{@zone}.#{@type}.#{@variant}.bmp"
  ...
  @load: (land_dir) ->
    new Promise (fulfill, reject) ->
      ...
      Promise.all(_.map(image_file_paths, (path) -> Jimp.read(path)))
      .then (images) ->
        progress = new ConsoleProgressUpdater(images.length)
        _.map(_.zip(image_file_paths, images), (pair) ->
          image = new LandTexture(land_dir, pair[0].substring(land_dir.length + 1), pair[1])
          progress.next()
          image
        )
      .then fulfill
